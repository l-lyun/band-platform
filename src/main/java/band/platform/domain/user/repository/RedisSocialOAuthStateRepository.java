...
public class RedisSocialOAuthStateRepository implements SocialOAuthStateRepository {
	...
	@Override
	public void save(SocialOAuthState oauthState, Duration ttl) {
		redisTemplate.opsForValue()
			.set(oauthStateKey(oauthState.state()), oauthState.nonce(), ttl);
	}

	@Override
	public Optional<SocialOAuthState> consume(String state) {
		String nonce = redisTemplate.execute(CONSUME_OAUTH_STATE_SCRIPT, List.of(oauthStateKey(state)));
	...
	private static String oauthStateKey(String state) {
